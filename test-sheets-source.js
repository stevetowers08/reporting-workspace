...
/**
 * Test Google Sheets API for 'source' column data
 */

import fetch from 'node-fetch';

// Test with a sample spreadsheet ID - you can replace this with an actual ID
...
async function testSheetsSource() {
  ...
  try {
    ...
    const response = await fetch('https://tulenreporting.vercel.app/api/google-sheets-data', {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      console.log('❌ Edge Function not available, trying direct API test...');
      await testDirectAPI();
      return;
    }
...
async function testDirectAPI() {
  console.log('🔍 Testing direct Google Sheets API...');
  
  // This would require a valid access token
  console.log('⚠️  Direct API test requires valid access token');
  console.log('   Check if Google Sheets integration is properly configured');
}

...
export const Option7LineChart: React.FC<{ data?: any }> = ({ data }) => {
  ...
  const chartData = {
    labels: ['Meta Ads', 'Google Ads'],
    datasets: [
      getLineDatasetConfig('Leads', [metaLeads, googleLeads], CHART_COLORS.primary),
      getLineDatasetConfig('Spend ($)', [metaSpend, googleSpend], CHART_COLORS.secondary),
    ],
  };

  const options = getDefaultChartOptions('Platform Performance Comparison');
...
export const Option8ScatterPlot: React.FC<{ data?: any }> = ({ data }) => {
  ...
  const options = getDefaultChartOptions('Spend vs Leads Analysis');

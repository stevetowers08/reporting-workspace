...
export const GHLFunnelAnalytics: React.FC<GHLFunnelAnalyticsProps> = ({ locationId, dateRange }) => {
  ...
  const funnelPerformanceData = funnelData.funnelBreakdown.map((funnel, index) => {
    // Use actual funnel data from API instead of estimates
    const funnelDetails = funnelData.funnels.find(f => f.name === funnel.name);
    const totalViews = funnelDetails ? 
      funnelDetails.pages.reduce((sum, page) => sum + page.views, 0) : 0;
    const totalConversions = funnelDetails ? 
      funnelDetails.pages.reduce((sum, page) => sum + page.conversions, 0) : 0;

...
export class GoogleSheetsOAuthService {
  ...
  static async generateSheetsAuthUrl(redirectUri?: string): Promise<string> {
    ...
    const params = new URLSearchParams({
      ...
      redirect_uri: redirectUri || (window.location.hostname === 'localhost' ? `${window.location.origin}/oauth/callback` : import.meta.env.VITE_OAUTH_REDIRECT_URL || 'https://tulenreporting.vercel.app/oauth/callback'),
      response_type: 'code',
    ...
    try {
      ...
      const tokenResponse = await fetch(this.GOOGLE_TOKEN_URL, {
        ...
        body: new URLSearchParams({
          ...
          redirect_uri: window.location.hostname === 'localhost' ? `${window.location.origin}/oauth/callback` : import.meta.env.VITE_OAUTH_REDIRECT_URL || 'https://tulenreporting.vercel.app/oauth/callback',

...
export class AgencyService {
  ...
  static async loadIntegrations(): Promise<IntegrationDisplay[]> {
    try {
      debugLogger.info('AgencyService', 'Loading integrations');
      
      // Use the new unified integration service
      const integrations = await UnifiedIntegrationService.getIntegrationDisplay();
      
      debugLogger.info('AgencyService', 'Loaded integrations', { count: integrations.length });
      return integrations;
    } catch (error) {
      debugLogger.error('AgencyService', 'Failed to load integrations', error);
  ...
  static async testConnection(platform: string): Promise<TestResult> {
    try {
      debugLogger.info('AgencyService', `Testing connection for ${platform}`);
      ...
      switch (platform) {
        ...
        case 'goHighLevel': {
          ...
          try {
            ...
            const hasValidToken = ghlIntegration?.config?.tokens?.accessToken && 
                                 ghlIntegration?.config?.tokens?.refreshToken;
      ...
      debugLogger.info('AgencyService', `Connection test result for ${platform}`, result);
      return result;
    } catch (error) {
      debugLogger.error('AgencyService', `Connection test failed for ${platform}`, error);
  ...
  static async connectIntegration(platform: string): Promise<void> {
    try {
      debugLogger.info('AgencyService', `Connecting to ${platform}`);
      ...
      if (platform === 'google-ai') {
        ...
        if (apiKey) {
          ...
          if (isValid) {
            ...
            debugLogger.info('AgencyService', 'Google AI Studio connected successfully');
          } else {
      ...
      // GoHighLevel is client-only - no agency connection needed
      if (platform === 'goHighLevel') {
        throw new Error('GoHighLevel connections are handled per-client, not at agency level');
      }

      // Special handling for Google Sheets - use the new dedicated OAuth service
      if (platform === 'googleSheets') {
        debugLogger.info('AgencyService', 'Connecting Google Sheets using dedicated OAuth service');
        ...
        debugLogger.info('AgencyService', 'Redirecting to Google Sheets OAuth', { authUrl });
      ...
      debugLogger.info('AgencyService', `Redirecting to ${platform} OAuth`, { authUrl });
      ...
      debugLogger.error('AgencyService', `Failed to connect ${platform}`, error);
      throw error;
    }
  }
  ...
  static async disconnectIntegration(platform: string): Promise<void> {
    try {
      debugLogger.info('AgencyService', `Disconnecting from ${platform}`);
      
      // Special handling for Google Sheets - use the dedicated OAuth service
      if (platform === 'googleSheets') {
        await GoogleSheetsOAuthService.disconnectSheets();
        debugLogger.info('AgencyService', 'Google Sheets disconnected successfully');
        return;
      }
      ...
      debugLogger.info('AgencyService', `${platform} disconnected successfully`);
    } catch (error) {
      debugLogger.error('AgencyService', `Failed to disconnect ${platform}`, error);
      throw error;
    }
  }
  ...
  static async createClient(clientData: {
    ...
    try {
      ...
      debugLogger.info('AgencyService', 'Client created successfully');
    } catch (error) {
      debugLogger.error('AgencyService', 'Failed to create client', error);
      throw error;
    }
  }
  ...
  static async updateClient(clientId: string, updates: Partial<Client>): Promise<void> {
    try {
      debugLogger.info('AgencyService', 'Updating client', { clientId });
      await DatabaseService.updateClient(clientId, updates);
      debugLogger.info('AgencyService', 'Client updated successfully');
    } catch (error) {
      debugLogger.error('AgencyService', 'Failed to update client', error);
      throw error;
    }
  }
  ...
  static async deleteClient(clientId: string): Promise<void> {
    try {
      debugLogger.info('AgencyService', 'Deleting client', { clientId });
      await DatabaseService.deleteClient(clientId);
      debugLogger.info('AgencyService', 'Client deleted successfully');
    } catch (error) {
      debugLogger.error('AgencyService', 'Failed to delete client', error);
      throw error;
    }
  }
}

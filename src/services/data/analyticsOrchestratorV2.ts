...
// Main Analytics Orchestrator V2
export class AnalyticsOrchestratorV2 {
  ...
  static startBackgroundRefresh(clientId: string, dateRange: DateRange): void {
    ...
    this.backgroundRefreshInterval = setInterval(async () => {
      try {
        debugLogger.info('AnalyticsOrchestratorV2', 'Starting background cache refresh');
        await this.cache.warmCache(clientId, dateRange);
        
        // Log cache statistics
        const stats = this.cache.getStats();
        debugLogger.info('AnalyticsOrchestratorV2', 'Cache statistics', stats);
      } catch (error) {
        debugLogger.error('AnalyticsOrchestratorV2', 'Background refresh failed', error);
      }
    }, 2 * 60 * 1000); // 2 minutes

    debugLogger.info('AnalyticsOrchestratorV2', 'Background refresh started', { clientId });
  }
  ...
  static stopBackgroundRefresh(): void {
    if (this.backgroundRefreshInterval) {
      ...
      debugLogger.info('AnalyticsOrchestratorV2', 'Background refresh stopped');
    }
  }
  ...
  static async getDashboardData(
    ...
    return this.deduplicator.deduplicate(cacheKey, async () => {
      debugLogger.info('AnalyticsOrchestratorV2', 'Fetching dashboard data', { clientId, dateRange });
      ...
      const [facebookData, googleData, ghlData, leadData] = await Promise.allSettled(promises);

      // Normalize and combine data - BEST PRACTICE: Simple, explicit handling
      const dashboardData: EventDashboardData = {
        clientData,
        facebookMetrics: facebookData.status === 'fulfilled' && facebookData.value ? facebookData.value : undefined,
        googleMetrics: googleData.status === 'fulfilled' && googleData.value ? googleData.value : undefined,
        ghlMetrics: ghlData.status === 'fulfilled' && ghlData.value ? ghlData.value : undefined,
        leadMetrics: leadData.status === 'fulfilled' && leadData.value ? leadData.value : undefined,
        totalLeads: 0,
        totalSpend: 0,
      ...
      debugLogger.info('AnalyticsOrchestratorV2', 'Final dashboard data', {
  ...
  /**
   * Get Facebook data with direct API calls and V2 improvements
   */
  private static async getFacebookData(
    ...
    return this.deduplicator.deduplicate(cacheKey, async () => {
      return facebookCircuitBreaker.execute(async () => {
        debugLogger.info('AnalyticsOrchestratorV2', 'Fetching Facebook data with V2 API calls', { 
          clientId, 
        ...
        // Build Facebook Insights API URL with proper parameters
            const params = new URLSearchParams({
              ...
              fields: 'impressions,clicks,spend,actions,ctr,cpc,cpm,reach,frequency,cost_per_link_click',
              level: 'account',
        ...
        const url = `https://graph.facebook.com/v18.0/${accountId}/insights?${params}`;
        
        // Make API call with V2 improvements
        const response = await this.makeFacebookApiCall(url);
        
        if (!response.data || response.data.length === 0) {
          debugLogger.warn('AnalyticsOrchestratorV2', 'Facebook API returned empty data', {
            accountId,
            dateRange,
            response
          });
        ...
        debugLogger.info('AnalyticsOrchestratorV2', 'Facebook breakdown data fetched', {
        ...
        try {
          ...
          console.warn('⚠️ Demographics processing failed, using empty data:', processingError);
          normalizedData.demographics = {
            ageGroups: { '25-34': 0, '35-44': 0, '45-54': 0, '55+': 0 },
            gender: { female: 0, male: 0 }
          };
          normalizedData.platformBreakdown = {
            facebookVsInstagram: { facebook: 0, instagram: 0 },
            adPlacements: { feed: 0, stories: 0, reels: 0 }
          };
        }
        
        debugLogger.info('AnalyticsOrchestratorV2', 'Facebook data normalized with breakdowns', {
        ...
        debugLogger.info('AnalyticsOrchestratorV2', 'Facebook data fetched successfully', {
  ...
  /**
   * Get Facebook access token from Supabase (using same method as V1)
   */
  private static async getFacebookAccessToken(): Promise<string | null> {
    try {
      ...
      if (error || !data) {
        debugLogger.warn('AnalyticsOrchestratorV2', 'No Facebook integration found', { error: error?.message });
        return null;
      }

      const config = data?.config;
      if (!config?.accessToken) {
        debugLogger.warn('AnalyticsOrchestratorV2', 'No Facebook access token found in config');
        return null;
      }

      debugLogger.info('AnalyticsOrchestratorV2', 'Facebook access token found from integrations table');
      return config.accessToken;
    } catch (error) {
      debugLogger.error('AnalyticsOrchestratorV2', 'Error getting Facebook access token', error);
      return null;
    }
  }

  /**
   * Make Facebook API call with V2 improvements (rate limiting, retry logic, error handling)
   */
  private static async makeFacebookApiCall(url: string, retryCount = 0): Promise<any> {
    ...
    try {
      debugLogger.debug('AnalyticsOrchestratorV2', `Facebook API request attempt ${retryCount + 1}/${MAX_RETRIES}`, { 
      ...
      const response = await fetch(url, {
        method: 'GET',
        headers: {
          'User-Agent': 'Marketing-Analytics-Dashboard-V2/1.0'
        },
      ...
      if (!response.ok) {
        ...
        if (response.status === 429 && retryCount < MAX_RETRIES) {
          ...
          debugLogger.warn('AnalyticsOrchestratorV2', `Facebook rate limited, retrying in ${delay}ms`, { retryCount });
          await new Promise(resolve => setTimeout(resolve, delay));
        ...
        if (response.status === 400 && errorData.error?.code === 190) {
          debugLogger.error('AnalyticsOrchestratorV2', 'Facebook token expired', errorData);
          throw new Error('Facebook access token expired');
        }

        debugLogger.error('AnalyticsOrchestratorV2', 'Facebook API error details', {
      ...
      debugLogger.debug('AnalyticsOrchestratorV2', 'Facebook API response received', {
      ...
      if (error.name === 'AbortError') {
        debugLogger.error('AnalyticsOrchestratorV2', 'Facebook API request timeout');
        throw new Error('Facebook API request timeout');
      }

      if (retryCount < MAX_RETRIES) {
        const delay = BASE_DELAY * Math.pow(2, retryCount);
        debugLogger.warn('AnalyticsOrchestratorV2', `Facebook API error, retrying in ${delay}ms`, { error, retryCount });
        await new Promise(resolve => setTimeout(resolve, delay));
        return this.makeFacebookApiCall(url, retryCount + 1);
      }

      debugLogger.error('AnalyticsOrchestratorV2', 'Facebook API request failed after retries', error);
      throw error;
    }
  }
  ...
  private static normalizeFacebookMetrics(rawData: any): FacebookMetricsWithTrends {
    // Extract leads from actions array (same as V1)
    let leads = 0;
    if (rawData.actions) {
      // Look for purchase or lead actions
      const action = rawData.actions.find((action: any) =>
    ...
    return {
      ...
      costPerLead: leads > 0 ? parseFloat(rawData.spend) / parseInt(leads.toString()) : 0,
      ctr: rawData.impressions > 0 ? (parseInt(rawData.clicks || '0') / parseInt(rawData.impressions || '1')) * 100 : 0, // Calculate CTR as percentage
      cpm: parseFloat(rawData.cpm || '0'),
      cpc: rawData.clicks > 0 ? parseFloat(rawData.spend) / parseInt(rawData.clicks) : 0, // Cost per click (same as cost per link click)
      demographics: {
        ageGroups: { '25-34': 0, '35-44': 0, '45-54': 0, '55+': 0 },
        gender: { female: 0, male: 0 }
      },
  ...
  private static async getFacebookDemographics(
    ...
    try {
      ...
      debugLogger.info('AnalyticsOrchestratorV2', 'Facebook demographics API response', {
        url: url.replace(/access_token=[^&]+/, 'access_token=***'),
      ...
      debugLogger.error('AnalyticsOrchestratorV2', 'Failed to fetch Facebook demographics', error);
      return [];
    }
  }
  ...
  private static async getFacebookPlatformBreakdown(
    ...
    try {
      ...
      debugLogger.error('AnalyticsOrchestratorV2', 'Failed to fetch Facebook platform breakdown', error);
      return [];
    }
  }
  ...
  private static processDemographicData(insightsData: any[]): any {
    ...
    // Convert to percentages (same as V1)
    if (totalLeads > 0) {
  ...
  /**
   * Process platform breakdown data (same logic as V1)
   */
  private static processPlatformBreakdownData(insightsData: any[]): any {
    ...
    insightsData.forEach((insight: any) => {
      ...
      // Process placements (feed, stories, reels) - use leads for consistency with V1
  ...
  /**
   * Extract leads from actions array (same logic as V1)
   */
  ...
  /**
   * Get empty Facebook metrics
   */
  private static getEmptyFacebookMetrics(): FacebookMetricsWithTrends {
    return {
      leads: 0,
      spend: 0,
      impressions: 0,
  ...
  /**
   * Get Google data with direct API calls and V2 improvements
   */
  private static async getGoogleData(
    ...
    return this.deduplicator.deduplicate(cacheKey, async () => {
      return googleCircuitBreaker.execute(async () => {
        debugLogger.info('AnalyticsOrchestratorV2', 'Fetching Google Ads data', {
          clientId,
        ...
        if (!mainMetrics) {
          debugLogger.warn('AnalyticsOrchestratorV2', 'Google Ads API returned no data');
        ...
        debugLogger.info('AnalyticsOrchestratorV2', 'Google Ads breakdown data fetched', {
        ...
        try {
          ...
          console.warn('⚠️ Google Ads processing failed, using empty data:', processingError);
          normalizedData.demographics = {
            ageGroups: { '25-34': 0, '35-44': 0, '45-54': 0, '55+': 0 },
            gender: { female: 0, male: 0 }
          };
          normalizedData.campaignBreakdown = {
  ...
  /**
   * Get empty Google metrics
   */
  private static getEmptyGoogleMetrics(): GoogleMetricsWithTrends {
    return {
      leads: 0,
      cost: 0,
      impressions: 0,
  ...
  /**
   * Get empty GoHighLevel metrics
   */
  private static getEmptyGoHighLevelMetrics(): GoHighLevelMetrics {
    return {
      totalContacts: 0,
      newContacts: 0,
   ...
   */
  private static normalizeLeadMetrics(rawData: any): LeadMetrics {
    return {
      totalLeads: rawData.totalLeads || 0,
      facebookLeads: rawData.facebookLeads || 0,
      googleLeads: rawData.googleLeads || 0,
      totalGuests: rawData.totalGuests || 0,
      averageGuestsPerLead: rawData.averageGuestsPerLead || 0,
      eventTypes: rawData.eventTypes || [],
      leadSources: rawData.leadSources || [],
      guestRanges: rawData.guestRanges || [],
      dayPreferences: rawData.dayPreferences || [],
      landingPageTypes: rawData.landingPageTypes || []
    };
  }

  /**
   * Get empty lead metrics
   */
  private static getEmptyLeadMetrics(): LeadMetrics {
    return {
      totalLeads: 0,
      facebookLeads: 0,
      googleLeads: 0,
  ...
  /**
   * Get GoHighLevel data with direct V2 API calls
   */
  private static async getGoHighLevelData(
    ...
    return this.deduplicator.deduplicate(cacheKey, async () => {
      return ghlCircuitBreaker.execute(async () => {
        debugLogger.info('AnalyticsOrchestratorV2', 'Fetching GoHighLevel data with V2 API calls', { 
          clientId, 
        ...
        if (!locationId) {
          debugLogger.warn('AnalyticsOrchestratorV2', 'No GoHighLevel locationId found');
          return undefined;
        }
        ...
        // Get comprehensive GHL metrics using V2 service
        ...
        if (!ghlMetrics) {
          debugLogger.warn('AnalyticsOrchestratorV2', 'GoHighLevel API returned no data');
          return this.getEmptyGoHighLevelMetrics();
        }
        ...
        debugLogger.info('AnalyticsOrchestratorV2', 'GoHighLevel data fetched successfully', {
  ...
  /**
   * Get lead data with direct V2 API calls
   */
  private static async getLeadData(
    clientId: string, 
    dateRange: DateRange, 
    clientData: Client
  ): Promise<LeadMetrics | undefined> {
    if (!clientData.accounts?.googleSheets || clientData.accounts.googleSheets === 'none') {
      return undefined;
    }
    ...
    return this.deduplicator.deduplicate(cacheKey, async () => {
      return sheetsCircuitBreaker.execute(async () => {
        debugLogger.info('AnalyticsOrchestratorV2', 'Fetching lead data with V2 API calls', { 
          clientId, 
        ...
        // Get lead data using V2 service
        ...
        if (!leadData) {
          debugLogger.warn('AnalyticsOrchestratorV2', 'Lead data API returned no data');
          return this.getEmptyLeadMetrics();
        }

        // Normalize the data to match expected interface
        const normalizedData = this.normalizeLeadMetrics(leadData);
        
        // Cache with dependencies
        this.cache.set(cacheKey, normalizedData, [`leads-${clientId}`, `sheets-${clientData.accounts.googleSheets}`]);
        
        debugLogger.info('AnalyticsOrchestratorV2', 'Lead data fetched successfully', {
          totalLeads: normalizedData.totalLeads,
          facebookLeads: normalizedData.facebookLeads,
          googleLeads: normalizedData.googleLeads,
          totalGuests: normalizedData.totalGuests
        });
  ...
  static invalidateCache(clientId?: string, dependency?: string): void {
    ...
    debugLogger.info('AnalyticsOrchestratorV2', 'Cache invalidated', { clientId, dependency });
  }
  ...
  static async getIntegrationStatus(): Promise<{
    ...
    const cached = this.cache.get(cacheKey, 3 * 60 * 1000); // 3 minute cache
    if (cached) return cached;
